...
├── src/                           # Astro site: / (home), /404, Header/Footer
├── public/
│   ├── _ui/                       # shared CSS/JS (base, theme tokens, ui.js)
...
- `catalog` — builds the site and serves it with the hardened nginx config on
  port 8080; also routes `/secret/` to the Node service. Read-only root, tmpfs for
