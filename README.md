...
- **Focused landing pages** (`/pdf/merge`, `/pdf/compress`, `/image/crop`,
  ...
  generated by `src/pages/<tool>/[tool].astro` for each service folder. `/image/*`
  renders the tool inline with `src/components/ImageTools.astro` (sidebar links, no
  iframe); `/pdf/*` and `/convert/*` still embed their app pre-selected with
  `?tool=<name>&embed=1` until they are migrated to the inline pattern. Keep each
  page's copy unique; do not reuse the toolbox blurbs.
